...
static class Messager {
    ...
    private static void InitiateMessages() {
        ...
        message = """
                   ...
                   Make strategic decisions to balance progress and sustainability. {0}
                   """; 
        AddMessage(key, message);
        ...
        key = "reward_with_token";
        message = """
                  Congratulations!
                  You have successfully completed the challenge and earned a reward.
                  Additionally, you earned a token for your outstanding performance!
                  Your reward: {0}
                  """;
        AddMessage(key, message);
    ...
    /*
     * PrintMessage(string key, string? variable = null, bool interactive = false):
     * Prints a message to the console based on the specified key.
     * If a variable is provided, it is included in the message.
     * If interactive is true, the method waits for user input before returning.
     */
    private static string GenerateMessage(string key, string? variable, bool interactive = false) {
    Console.Clear();
    ...
    public static string InteractiveMessage(string key, string? variable = null, bool raw = false) {
        return GenerateMessage(key, variable, true); 
    }
